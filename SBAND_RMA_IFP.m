...
load sif2;
sif=sif2;
delta_x = convlength([2 / 12.0],'ft','m');
Rs=30.0;%"Downrange distance (ft) to calibration target at scene center (default: 30)",
...
center_freq = bandwidth / 2 + fstart;
Kr = linspace(((4 * pi / C) * (center_freq - bandwidth / 2)),...
    ((4 * pi / C) * (center_freq + bandwidth / 2)),  M );
...
zpad = 2048*2; %cross range symetrical zero pad
...
S_img1=dbv(S_image);
figure(50),imagesc(S_img1);
cr1 = -300; %(ft)
cr2 = 300; %(ft)
dr1 = round(1 + Rs/.3048); %(ft)
dr2 = round(350 + Rs/.3048); %(ft)
%data truncation
